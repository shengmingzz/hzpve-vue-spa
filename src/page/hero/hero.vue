    ...
    <div v-show="menu==0">
      <div class="skill" v-for="(item,key) in skills" :key="key">
        ...
        <div class="skillinfo">
          ...
          <div class="skillother">{{item.description2}}</div>
        </div>
      </div>
    </div>

    <div class="story" v-show="menu==1">
      {{hero.description}}
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    loadData () {
      this.hero = window.CosBox.getHeroDetail(this.code)
      this.skills = window.CosBox.getHeroSkill(this.hero.skills)
    },
    clickMenu (code) {
      this.menu = code
    }
